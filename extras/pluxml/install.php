...
if(! is_file(PLX_ROOT.'config.php')) {
	$content = <<< CONTENT
<?php
define('PLX_CONFIG_PATH', 'data-'.strftime('%F').'/configuration/');
?>
CONTENT;
	file_put_contents(PLX_ROOT.'config.php', $content);
}
...
if(!defined('PLX_VERSION')) {
	$version = (is_file('version')) ? trim(file_get_contents('version')) : preg_replace('@^.*/PluXml-@', '', dirname($_SERVER["PHP_SELF"]));
	define('PLX_VERSION', $version);
}
$target = preg_replace('@configuration/$@', '', PLX_CONFIG_PATH);
...
# Vérification de la racine des données
if(!is_dir(PLX_ROOT.$target) and mkdir(PLX_ROOT.$target, 0755, true)) {
	file_put_contents(PLX_ROOT.$target.'.htaccess', 'options -indexes');
	file_put_contents(PLX_ROOT.$target.'index.html', '');
}

# Vérification de certains dossiers et de leur accessibilité
define('HTACCESS', "<Files *>\n\tOrder allow,deny\n\tDeny from all\n</Files>\n");
foreach(explode(' ', 'articles commentaires configuration statiques') as $folder) {
	$dir1 = PLX_ROOT.$target.$folder;
	if(!is_dir($dir1) and mkdir($dir1, 0755, true)) {
		file_put_contents($dir1.'/.htaccess', HTACCESS);
	}
}

# Vérification de l'existence des dossiers médias
if(!is_dir(PLX_ROOT.$target.'medias')) {
	@mkdir(PLX_ROOT.$target.'medias',0755,true);
}

# Vérification de l'existence du dossier PLX_CONFIG_PATH.'plugins'
...
$config = array('title'=>'PluXml',
				...
				'racine_articles'=>$target.'articles/',
				'racine_commentaires'=>$target.'commentaires/',
				'racine_statiques'=>$target.'statiques/',
			...
			<form action="install.php" method="post">
				...
				<fieldset>
					...
					<ul class="unstyled-list">
						...
						<?php if(version_compare(PLX_VERSION, '5.6', '>='))
							plxUtils::testLibXml();
						?>
